...
uint128 hash64(const void * key, int nKeyBytes, uint32_t seed) {
	uint64_t hash[2]{};
	MurmurHash3::murmur_x64_128(key, nKeyBytes, seed, &hash);
	return uint128{ hash[0], hash[1] };
}

uint128 hash64(const std::string & key, uint32_t seed) {
	uint64_t hash[2]{};
...
//--------------------------------------------------------------------------
// This code has been adapted from Peter Scott's C port of the original
// algorithm.
