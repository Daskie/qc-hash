...
TEST(set, capacityConstructor) {
    ...
    EXPECT_EQ(size_t(  32u), qc_hash_chunk::Set<int>(  17u).capacity());
    EXPECT_EQ(size_t(  32u), qc_hash_chunk::Set<int>(  32u).capacity());
    EXPECT_EQ(size_t(  64u), qc_hash_chunk::Set<int>(  33u).capacity());
    EXPECT_EQ(size_t(  64u), qc_hash_chunk::Set<int>(  64u).capacity());
...
TEST(set, moveConstructor) {
    ...
    qc_hash_chunk::Set<int> s2(std::move(s1));
    EXPECT_EQ(ref, s2);
...
TEST(set, rangeConstructor) {
    ...
    qc_hash_chunk::Set<int> s(values.cbegin(), values.cend());
...
TEST(set, copyAssignment) {
    ...
    qc_hash_chunk::Set<std::string> s3{};
    for (int i{0}; i < 128; ++i) s3.emplace(std::to_string(i));
    qc_hash_chunk::Set<std::string> s4{};
    s4 = s3;
    EXPECT_EQ(s3, s4);
...
TEST(set, clear) {
    ...
    qc_hash_chunk::Set<std::string> s2;
...
TEST(set, emplace) {
    ...
    qc_hash_chunk::Set<Uncopiable, UncopiableHash> s;
    for (int i{0}; i < 128; ++i) {
...
std::vector<std::pair<int, int>> toElementVector(const qc_hash_chunk::Set<int> & s) {
...
TEST(set, noPreemtiveRehash) {
    ...
    EXPECT_EQ(qc_hash_chunk::config::minCapacity, s.capacity());
    s.emplace(int(qc_hash_chunk::config::minCapacity - 1));
    EXPECT_EQ(qc_hash_chunk::config::minCapacity, s.capacity());
    s.emplace(int(qc_hash_chunk::config::minCapacity - 1));
    EXPECT_EQ(qc_hash_chunk::config::minCapacity, s.capacity());
}
...
TEST(set, iterator) {
    ...
    qc_hash_chunk::Set<int> t;
    qc_hash_chunk::Set<int>::iterator it1(t.begin());
    qc_hash_chunk::Set<int>::const_iterator cit1 = t.cbegin();
    //it1 = cit1;
    cit1 = it1;
    qc_hash_chunk::Set<int>::iterator it2(it1);
    it2 = it1;
    qc_hash_chunk::Set<int>::const_iterator cit2(cit1);
    cit2 = cit1;
    qc_hash_chunk::Set<int>::iterator it3(std::move(it1));
    it3 = std::move(it1);
    qc_hash_chunk::Set<int>::const_iterator cit3(std::move(cit1));
    cit3 = std::move(cit1);
    it1 == cit1;
    cit1 == it1;
}
...
SetStats calcStats(const qc_hash_chunk::Set<V, H> & set) {
...
TEST(set, stats) {
    ...
    qc_hash_chunk::Set<int> s1(size);
    qc_hash_chunk::Set<int, MurmurHash> s2(size);
...
template <typename K> using RecordSet = qc_hash_chunk::Set<K, qc_hash_chunk::Hash<K>, std::equal_to<K>, qc::RecordAllocator<K>>;
...
TEST(set, sensitivity) {
    ...
    qc_hash_chunk::Set<Sensitive, SensitiveHash> s;
...
TEST(set, copyAversion) {
    ...
    EXPECT_FALSE(Tracker::copies());
    qc_hash_chunk::Map<Tracker, Tracker, TrackerHash> m2(std::move(m));
    EXPECT_FALSE(Tracker::copies());
    m = std::move(m2);
    EXPECT_FALSE(Tracker::copies());
